...
def calcular_risco(sexo_feminino, osteoporose, cimento, soltura):
    # Cálculo proxy baseado nos Odds Ratios do estudo de Chen et al. (2024)
    pontos = 0.0
    contribs = {}
    ...
    # Conversão para probabilidade de risco em 12 meses (Ajustado para uma incidência máxima aproximada de 15%)
    prob = min(99.9, (pontos / 8.0) * 15.0)
    
    return round(prob, 1), contribs

def renderizar_ui():
    st.markdown("<div class='calc-info'><b>O que avalia:</b> Estima o risco de <b>Fratura Periprotésica em 12 meses</b> após artroplastia da anca/quadril em pacientes muito idosos (≥ 80 anos). O modelo permite estratificar o risco para otimizar o tratamento médico e cirúrgico.</div>", unsafe_allow_html=True)
    ...
    • <b>Osteoporose:</b> A perda da densidade e da microarquitetura trabecular reduz a resistência mecânica do fémur proximal.<br>
    • <b>Prótese Não Cimentada:</b> A inserção sob pressão ("press-fit") gera uma tensão de expansão transversal ("hoop stress") num osso cortical já enfraquecido, aumentando drasticamente o risco de fissuras iatrogénicas.<br>
    ...
    if st.button("Calcular Risco de Fratura Periprotésica", key="btn_ppf"):
        ...
        salvar_registro("Fratura Periprotésica (Anca)", res, "complicacao", params_str)
